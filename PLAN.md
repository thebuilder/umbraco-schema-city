...
schema-city/
  ...
  src/
    SchemaCity/                      Razor Class Library, the NuGet package
      ...
      Client/
        package.json  vite.config.ts  tsconfig.json  openapi-ts.config.ts
...
`Client/dev/index.html` mounts `<schema-city-map>` with a fixture loader and a fake API. Fixtures: `small.json` (12 types), `medium.json` (80 types, folders, compositions, blocks), `pathological.json` (cycles, self-allowed folders, 40 element types, orphan types). Exported from the seeded test site with a one-line script so fixtures stay honest.
...
| End to end | Open the seeded site by hand at each milestone exit. No browser automation until a regression justifies it |
