...
| Click | select a building: unrelated nodes and edges fade to 20%, inspector opens. Clicking bare ground clears the selection (fix in progress) |
...
- Ground click clears the selection (in progress).
...
| Measured at the spike: 179 kB gzipped for the workspace entry, 340 kB for the lazy scene chunk, mostly drei | Chunk splitting brings the eager load to 1.2 kB for the workspace entry, 1.4 kB for the document-type-view entry, 0.7 kB for `api.js`, 52 kB of app and 185 kB of vendor, and the 348 kB scene chunk loads only when the city renders. Revisit drei imports at M1 exit; importing controls from `three/addons` directly is the fallback if 348 kB proves to matter. |
...
| `model/`, `app/` | 155 vitest tests across 15 files on fixtures: determinism (same input twice), cycle handling, empty graph, 300-node timing under 200 ms with realistic back edges, findings rules |
...
6. M3 tidy-up, usage in the wrappers. Done. Backoffice check of the editor tab, drawer and lens. Done. Editor link fixed and rechecked. Then M4: packaging and states. Done. World stage. Done. Roof icons, windows, Explore camera, list view, palette, focus plates, camera fix, the controls page. Done. Districts as islands with labels, road routing along streets: in progress. Next.
