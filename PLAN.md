...
| District | One padded island per district: slab and rim, colour shifted by kind (compositions lighter, elements warmer), the name printed flat on the ground on the largest empty rectangle the scene finds on the plate (no building, no building shadow, no road or link), along an edge first and in the interior otherwise, right way up from the default azimuth, left-aligned inside a band the layout holds empty (cap 4 units, shrinking to 2.5 when no spot holds the full size; only when nothing fits does a reserved band on the north edge take it), one canvas texture per district, phosphor-dim, no depth write, under the roads like silkscreen under traces; it is a fixed element of the scene and does not turn with the camera; a name wider than its island shrinks to fit; a nested folder is a lighter rectangle under its members |
