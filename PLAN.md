...
6. Positions are world units throughout: a building footprint is 2 units, two buildings in a row sit one and a half footprints apart, and a street is 9 units.
...
1. Roads follow the streets. A road leaves its parent's face, drops to the street between the parent's row and the child's row, runs along that street to the child's column and enters the child's face, so every run is north-south or east-west. Straight ribbons between building centres crossed 292 times on the seeded schema, which reads as wiring rather than as a city; the same 64 roads routed this way cross 117 times (94 once the three-parent fan limit hides the promiscuous children in the overview).
...
| District | One padded island per district: slab and rim, colour shifted by kind (compositions lighter, elements warmer), the name printed flat on the ground along the north edge, left-aligned in the padding at a cap height of 3 world units, one canvas texture per district (weight 600, 0.32em tracking), phosphor-dim at 0.8, no depth write, under the roads like silkscreen under traces; a name wider than its island shrinks to fit; the stamp yaws to the camera's azimuth every frame and its height is divided by the sine of the elevation, so it reads horizontal at 3 units on screen from any angle; a nested folder is a lighter rectangle under its members |
...
- Stress fixture `pathological.json` (300 types). Done 2026-09-03; it lays out in about 80 ms and exposed two layout defects, sparse ranks costing full bands and the unreserved stamp band, both in progress.
...
| `model/`, `app/` | 183 vitest tests across 16 files on fixtures: determinism (same input twice), cycle handling, empty graph, 300-node timing under 200 ms with realistic back edges, findings rules |
...
| Performance | The seeded schema in the dev harness with the browser's own frame profiler; `pathological.json` (300 types) lays out in about 80 ms and is the benchmark |
...
6. M3 tidy-up, usage in the wrappers. Done. Backoffice check of the editor tab, drawer and lens. Done. Editor link fixed and rechecked. Then M4: packaging and states. Done. World stage. Done. Roof icons, windows, Explore camera, list view, palette, focus plates, camera fix, the controls page. Done. Districts as islands with labels. Done. Road routing along streets. Done. Toolbar polish. Done. Stamped district names. Done. Toolbar wrap fix. Done. Keyboard flight and the stamp orientation. Done. In progress: compacting sparse ranks and reserving the stamp band. Next: screenshots for the README and the marketplace, then a final verification on both majors.
