...
6. Positions are world units throughout: a building footprint is 2 units, two buildings in a row sit one and a half footprints apart, and a street is 9 units. An island's padding is 3 units, except along the name's edge, where it is the computed name band.
...
| District | One padded island per district: slab and rim, colour shifted by kind (compositions lighter, elements warmer), the name printed flat on the ground along whichever island edge the fewest roads and links cross (ties broken north, west, south, east), aligned with that edge and right way up from the default azimuth, left-aligned inside a band the layout holds empty (cap 4 units; the band is the cap plus the ground the island's tallest first-row building hides at the isometric angle plus a unit of margin, at least 8 units, so no row obscures the letters), one canvas texture per district, phosphor-dim, no depth write, under the roads like silkscreen under traces; it is a fixed element of the scene and does not turn with the camera; a name wider than its island shrinks to fit; a nested folder is a lighter rectangle under its members |
...
- District stamp containment. Done 2026-09-04; fixed along the edge, placed once, band 4 units; the band is now computed from the tallest first row with a unit of margin, at least 8 units, and the name moves to the island edge with the fewest road crossings (in progress).
